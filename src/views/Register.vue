      ...
      <el-form ref="ruleFormRef" :model="form" :rules="rules" status-icon>
        ...
        <el-form-item prop="emailCode">
          <div style="display: flex">
            <el-input style="flex: 1" v-model="form.emailCode" clearable></el-input>
            <el-button style="width: 100px;margin-left: 10px" @click="sendEmail" :disabled="time>0">点击发送<span v-if="time">({{ time }})</span></el-button>
          </div>
        </el-form-item>
        <el-form-item>
...
const register = () => {
  ruleFormRef.value.validate(valid => {
    if (valid) {
      request.post("http://localhost:9090/register", form).then(res => {
        if (res.code === '200') {
